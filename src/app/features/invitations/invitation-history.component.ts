  ...
  styles: [
    `
      ...
      .invitation-item-card {
        @apply overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all hover:shadow-md;
      }
    `,
  ],
})
export class InvitationHistoryComponent implements OnInit {
  ...
  loadHistory(): void {
    ...
    forkJoin({
      accepted: this.spaceInvitationService.getMyInvitations({
        Status: InvitationStatus.Accepted,
        PageIndex: 1,
        PageSize: 50,
      }),
      rejected: this.spaceInvitationService.getMyInvitations({
